...
**Python**: 3.12推奨（`C:\Users\tsuch\AppData\Local\Programs\Python\Python312\python.exe`）
**OS**: Windows（JRA-VANアプリとのCOM連携のためWindows必須）

> **注意（2026-04-07）**: Gドライブ移行後、旧 `.venv` / `.venv32` は旧パスを参照して壊れている。`.venv_new` を使うこと。
> PowerShell経由での実行例:
> ```
> powershell -Command "cd 'G:\マイドライブ\horse_racing_ai'; & '.venv_new\Scripts\python.exe' src/_daily_roi_2026.py"
> ```
