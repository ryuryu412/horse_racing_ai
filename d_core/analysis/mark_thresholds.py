...
def roi_both(sub):
    ...
    pr = (sub['_atch'] <= 3).mean()
    return {'N': n, '勝率': wr, '複勝率': pr, '単ROI': roi_t, '複ROI': roi_f}

od_thresholds = [2, 3, 4, 5, 6, 8, 10]

gap_conditions = [
    ('なし（全体）',  lambda t: pd.Series([True]*len(t), index=t.index)),
    ('gap≥2x',       lambda t: t['gap_ratio'] > 2),
    ('gap≥3x',       lambda t: t['gap_ratio'] > 3),
    ('gap≥5x',       lambda t: t['gap_ratio'] > 5),
    ('1頭抜け',      lambda t: (t['D_pct'] > 200) & (t['_n_qual'] == 1)),
    ('gap≥3x|1頭抜け', lambda t: (t['gap_ratio'] > 3) | ((t['D_pct'] > 200) & (t['_n_qual'] == 1))),
]

# ── 単勝ROI最適表 ──
print("=" * 90)
print("【単勝ROI グリッド】")
hdr = f"  {'条件':<22}  {'N':>5}  {'勝率':>6}  {'単ROI':>8}  {'複ROI':>8}"
sep = "  " + "-" * 60
print(hdr); print(sep)

tan_results = []
for od in od_thresholds:
    for gap_name, gap_fn in gap_conditions:
        mask = (top1['odds'] > od) & gap_fn(top1)
        r = roi_both(top1[mask])
        if r is None: continue
        tan_results.append({'OD': od, 'gap': gap_name, **r})
        print(f"  OD>{od} & {gap_name:<16}  {r['N']:>5}  {r['勝率']:>6.1%}  {r['単ROI']:>+8.1%}  {r['複ROI']:>+8.1%}")
    print(sep)

# ── 複勝ROI最適表 ──
print()
print("=" * 90)
print("【複勝ROI グリッド】（同データ・複ROI降順Top20）")
print(hdr); print(sep)

df_tan = pd.DataFrame(tan_results)
if not df_tan.empty:
    df_tan_sorted = df_tan.sort_values('複ROI', ascending=False).head(20)
    for _, row in df_tan_sorted.iterrows():
        print(f"  OD>{int(row['OD'])} & {row['gap']:<16}  {int(row['N']):>5}  {row['勝率']:>6.1%}  {row['単ROI']:>+8.1%}  {row['複ROI']:>+8.1%}")

# ── 単勝ROI Top10 ──
print()
print("=" * 90)
print("【単勝ROI Top10】（N≥30）")
print(hdr); print(sep)
if not df_tan.empty:
    df_tan_sorted2 = df_tan.sort_values('単ROI', ascending=False).head(10)
    for _, row in df_tan_sorted2.iterrows():
        print(f"  OD>{int(row['OD'])} & {row['gap']:<16}  {int(row['N']):>5}  {row['勝率']:>6.1%}  {row['単ROI']:>+8.1%}  {row['複ROI']:>+8.1%}")

# ── バランス型（単複両方+20%以上）──
print()
print("=" * 90)
print("【バランス型】単ROI>+100% & 複ROI>+50% & N≥50")
print(hdr); print(sep)
if not df_tan.empty:
    bal = df_tan[(df_tan['単ROI'] > 1.0) & (df_tan['複ROI'] > 0.5) & (df_tan['N'] >= 50)]
    for _, row in bal.sort_values('N', ascending=False).iterrows():
        print(f"  OD>{int(row['OD'])} & {row['gap']:<16}  {int(row['N']):>5}  {row['勝率']:>6.1%}  {row['単ROI']:>+8.1%}  {row['複ROI']:>+8.1%}")
