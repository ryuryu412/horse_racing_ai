...
for rf in result_csvs:
    ...
    for gk, idx in merged.groupby(['場所', 'Ｒ'], sort=False).groups.items():
        ...
        for i, (row_idx, row) in enumerate(sub.iterrows()):
            records.append({'着_num': row['着_num'], '強さPT': pt.iloc[i]})

df = pd.DataFrame(records).dropna(subset=['着_num', '強さPT'])

bins   = [43, 45, 47, 50, 53, 55, 57, 60, 65, 70, 110]
labels = ['43-45','45-47','47-50','50-53','53-55','55-57','57-60','60-65','65-70','70~']

for place in [1, 2, 3]:
    s = df[df['着_num'] == place]['強さPT']
    cut = pd.cut(s, bins=bins, labels=labels)
    print(f'=== {place}着 (計{len(s)}頭) ===')
    print(f'  最小:{s.min():.1f} / 中央値:{s.median():.1f} / 平均:{s.mean():.1f} / 最大:{s.max():.1f}')
    for label, cnt in cut.value_counts().sort_index().items():
        pct = cnt / len(s) * 100
        bar = '#' * int(pct / 2)
        print(f'  {label:>8}: {cnt:3d}頭 ({pct:4.1f}%) {bar}')
    print()
