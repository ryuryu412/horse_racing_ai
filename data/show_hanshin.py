...
hanshin = data[data['場名'] == '阪神'].sort_values('レース')

with open('C:/Users/tsuch/Desktop/horse_racing_ai/data/hanshin_0322.txt', 'w', encoding='utf-8') as fw:
    for race, grp in hanshin.groupby('レース'):
        buy = grp['購入_n'].sum()
        pay = grp['払戻_n'].sum()
        fw.write(f'【阪神 {race}R】 購入:{buy:,}円  払戻:{pay:,}円\n')
        for _, r in grp.iterrows():
            hit = str(r['的中／返還']).strip()
            fw.write(f'  {str(r["式別"])[:16]:16} {str(r["馬／組番"])[:22]:22} 購入:{r["購入_n"]:>6,}円  払戻:{r["払戻_n"]:>7,}円  [{hit}]\n')
        fw.write('\n')
    fw.write(f'阪神合計 購入:{hanshin["購入_n"].sum():,}円  払戻:{hanshin["払戻_n"].sum():,}円\n')
print('done')
